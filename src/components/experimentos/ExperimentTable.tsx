...
export function ExperimentTable({ columns, data, filtered, hiddenColumns, selectedIdx, setSelectedIdx, handleEdit, toggleColumn, optionsMap, setData, setFiltered }: ExperimentTableProps) {
  const [historyModal, setHistoryModal] = React.useState<{row: any, col: string} | null>(null);

  // Função para editar situação atual e salvar histórico
  const [comentarioDraft, setComentarioDraft] = React.useState<{[id: string]: string}>({});
  ...
  async function handleSalvarComentario(row: any) {
    ...
    const historico = Array.isArray(row['Comentários/Pendências e Ações']) ? [...row['Comentários/Pendências e Ações']] : [];
    ...
    if (res.ok) {
      ...
      setFiltered(filtered.map((item) => item._id === row._id ? atualizado : item));
              ...
              {columns.map((col) => (
                <TableCell key={col}>
                  {col === 'Comentários/Pendências e Ações' ? (
                    <div>
                      {row['Situação Atual e Próximos passos'] || row['Situacao Atual e Proximos passos'] || row['Situacao Atual'] ? (
                        <div className="mb-1 text-sm font-semibold text-gray-800">
                          Situação atual: {row['Situação Atual e Próximos passos'] || row['Situacao Atual e Proximos passos'] || row['Situacao Atual']}
                        </div>
                      ) : (Array.isArray(row[col]) && row[col].length > 0 && (
                        <div className="mb-1 text-sm font-semibold text-gray-800">
                          Situação atual: {row[col][row[col].length - 1].texto}
                        </div>
                      ))}
                      <input
                        type="text"
                        ...
                        onChange={e => setComentarioDraft((prev) => ({ ...prev, [row._id]: e.target.value }))}
                        onClick={e => e.stopPropagation()}
                      />
                      {comentarioDraft[row._id] && comentarioDraft[row._id].trim() && (
                        <button
                          className="ml-2 px-2 py-1 bg-blue-600 text-white rounded text-xs"
                          onClick={e => { e.stopPropagation(); handleSalvarComentario(row); }}
                        >Salvar</button>
                      )}
                      <button
                        className="text-xs text-blue-600 underline mt-1 ml-2"
                        onClick={e => { e.stopPropagation(); setHistoryModal({row, col}); }}
                    ...
                    <InlineDropdown
                      ...
                      onChange={async (novoValor) => {
                        ...
                        if (res.ok) {
                          setData(data.map((item) => item._id === row._id ? atualizado : item));
                          setFiltered(filtered.map((item) => item._id === row._id ? atualizado : item));
                          window.alert(`Campo "${col}" atualizado com sucesso!`);
                        }
                      }}
                    />
                  ) : col === 'Iniciativa' ? (
      ...
      {historyModal && (
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full relative">
            <button className="absolute top-2 right-2 text-gray-500 hover:text-red-600" onClick={() => setHistoryModal(null)}>Fechar</button>
            <h2 className="text-xl font-bold mb-4">Histórico de Situação Atual</h2>
