...
export function useExperimentos() {
  ...
  const totalProspeccao = getCount(
  ...
  const totalConcluidoGoNoGo = data.filter(
    (item) =>
      typeof item["Experimentação"] === "string" &&
      item["Experimentação"]
        .toLowerCase()
        .includes("concluido - aguardando go/no")
  ).length;
  const pilotosAndamento = getCount("Piloto", "em andamento");
  const pilotosConcluidos = data.filter(
    (item) =>
      typeof item["Piloto"] === "string" &&
      item["Piloto"].toLowerCase().includes("concluido")
  ).length;
  ...
  const experimentosPorEtapa = etapas.map((etapa) => ({
    name: etapa,
    value: data.filter(
      (item) => typeof item[etapa] === "string" && item[etapa].trim() !== ""
    ).length,
    percentage: 0,
  }));
  ...
  const experimentosPorTipo = tipos.map((tipo) => ({
    ...
    value: data.filter(
      (item) =>
        typeof item["Tecnologia"] === "string" &&
        item["Tecnologia"].trim() === tipo.trim()
    ).length,
  ...
  data.forEach((item) => {
    let mesConclusao =
      typeof item["Mês Conclusão"] === "string"
        ? item["Mês Conclusão"].trim()
        : undefined;
    let anoConclusao =
      typeof item["Ano Conclusão"] === "string"
        ? item["Ano Conclusão"].trim()
        : undefined;
    ...
    if (mesConclusao && anoConclusao && meses.includes(mesConclusao)) {
      ...
      mesesAnos[anoConclusao][mesConclusao] =
        (mesesAnos[anoConclusao][mesConclusao] || 0) + 1;
    }
  });
  ...
  const experimentosPorMes: ExperimentoPorMes[] = meses.map((mes) => {
    const obj: ExperimentoPorMes = { month: mes };
    anos.forEach((ano) => {
