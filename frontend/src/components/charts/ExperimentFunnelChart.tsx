...
export const ExperimentFunnelChart: React.FC<ExperimentFunnelChartProps> = ({
  stages,
}) => {
      ...
      <div className="flex flex-col md:flex-row gap-8 md:gap-12 items-center w-full justify-center">
        ...
        <div className="flex flex-col gap-4 items-center w-full max-w-[420px] relative px-2">
          {stagesWithConversion.map((stage, idx) => (
            <div
              ...
              style={{
                marginBottom: idx < stagesWithConversion.length - 1 ? 0 : 0,
              }}
            >
              <div
        ...
        <div className="flex flex-col gap-4 w-full max-w-[400px] px-2">
          {stagesWithConversion.slice(1).map((stage, idx) => (
            <div
              key={stage.name}
              className="bg-orange-50 rounded-xl px-4 py-3 shadow-lg text-gray-800 text-base font-semibold flex flex-col gap-1 border border-orange-200"
            >
              <span>
                <b className="text-orange-600 text-lg">{stage.conversion}%</b>{" "}
                convertidos para <span className="font-bold">{stage.name}</span>
              </span>
